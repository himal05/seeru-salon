...
export default function SalonSite() {
  ...
  return <main>
    ...
    <section className="testimonials section"><SectionHead eyebrow="WHAT OUR CLIENTS SAY" title="हाम्रा ग्राहकहरूको अनुभव ❤️"/><div className="testimonial-wrap"><div className="quote-mark">“</div><div className="stars" aria-label="5 out of 5 stars">{[1,2,3,4,5].map(i=><Star key={i} size={17} fill="currentColor"/>)}</div><blockquote>{testimonials[testimonial].quote}</blockquote><div><strong>{testimonials[testimonial].name}</strong><span>{testimonials[testimonial].service}</span></div><div className="dots">{testimonials.map((review,i)=><button aria-label={`Show review by ${review.name}`} onClick={()=>setTestimonial(i)} className={testimonial===i?"active":""} key={review.name}/>)}</div></div><div className="google-card"><div><span className="google-g">G</span><div><small>GOOGLE REVIEWS</small><strong>4.9 <span>★★★★★</span></strong><p>Based on [250+] client reviews</p></div></div><a href="#">View Google reviews <ArrowRight size={15}/></a></div></section>
